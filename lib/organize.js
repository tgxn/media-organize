...
class Organize {
    constructor(configLocations) {
        ...
        this.store = new Store(configLocations.storage);
    }
        ...
        for (const index in this.configArray) {
            ...
            for (const index in directories) {
                ...
                watcher.on("create", (file, stats) =>
                    layer.onFileCreated(file, stats),
                );
...
class OrganizerLayer {
    ...
    async organizeDirectory() {
        ...
        if (
            this.configData.hasOwnProperty("enabled") &&
                ...
                this.getFiles(scanDir).then(async (files) => {
                    ...
                    const results = await Promise.all(
                        files.map(async (filePath) => {
                            ...
                            const fileIsMedia = await this.isAllowedFile(
                                filePath,
                            );

                            if (fileIsMedia) {
                                const movieOrSeries =
                                    await this.isMovieOrSeries(parsedPath);

                                if (
                                    this.configData.hasOwnProperty("strictType")
                                ) {
                                    if (
                                        movieOrSeries !==
                                        this.configData.strictType
                                    ) {
                                        console.log(
    ...
    async isAllowedFile(filePath) {
        ...
        const isInArray = (extensionsArray, fileExtension) => {
            ...
            for (const item of extensionsArray) {
                if (
                    item.toLowerCase().indexOf(fileExtension.toLowerCase()) !==
                    -1
                ) {
                    found = true;
                }
        ...
        if (
            this.configData.hasOwnProperty("allowedSize") &&
            Array.isArray(this.configData.allowedSize)
        ) {
    ...
    async determineMediaInfo(fileName, typeDetermination) {
        ...
        console.log(
            "failed to determine media data",
            fileName,
            typeDetermination,
        );
        return null;
    ...
    formatSeriesPath(targetMetadata, fileInfo) {
        ...
        if (
            ...
            targetMetadata.name = Case[this.configData.seriesCaseFormat](
                targetMetadata.name,
            );
        }
                        ...
                        if (metaData.hasOwnProperty("quality")) {
                            ...
                            if (searchLink.metaData.hasOwnProperty("quality")) {
                                const existingQualityInteger =
                                    typeof searchLink.metaData.quality ===
                                    "string"
                                        ? searchLink.metaData.quality.replace(
                                              /\D/g,
                                              "",
                                          )
