    ...
    async registerWatchers() {
        ...
        for (const index in this.configArray) {
            ...
            for (const index in directories) {
                ...
                spinnies.add(`watcher-${index}-${index}`, { text: `Watching "${scanDir}"...` });
...
class OrganizerLayer {
    ...
    constructor(organizeParent, configIndex) {
        ...
        this.store = organizeParent.store;
    ...
    async onFileDeleted(file) {
        ...
        const linkExists = this.store.findLinkWithSource(file);
        if (linkExists) {
            console.log("Remove Link:", linkExists.linkPath);

            try {
                await fs.unlink(linkExists.linkPath);
                this.store.deleteLink(linkExists.linkPath);
            } catch (error) {
    ...
    async organizeDirectory() {
        spinnies.add(this.configIndex, { text: `Running Config ${this.configIndex} (${this.directories.length} directories)` });

        if (this.configData.hasOwnProperty("enabled") && this.configData.enabled === false) {
            spinnies.fail(this.configIndex, { text: `Config ${this.configIndex} is disabled! (${this.directories.length} directories)` });
            return "disabled";
        }
            ...
            directoryPromises.push(
                this.getFiles(scanDir)
                    .then(async files => {
                        ...
                        spinnies.succeed(scanDir, { text: `Organized "${scanDir}"! (${results.length} files)` });
                    })
            );
        }
        ...
        spinnies.succeed(this.configIndex, { text: `Config ${this.configIndex} completed! (${this.directories.length} directories)` });
    ...
    async isAllowedFile(filePath) {
        ...
        if (this.configData.hasOwnProperty("allowedExtensions") && this.configData.allowedExtensions.length !== 0) {
        ...
        if (this.configData.hasOwnProperty("ignoredExtensions") && isInArray(this.configData.ignoredExtensions, fileExt)) {
            allowedFile = false;
        }
    ...
    async isMovieOrSeries(parsedPath) {
        ...
        if (parseFileName) {
            if (
                (parseFileName.hasOwnProperty("episode") && parseFileName.episode && parseFileName.hasOwnProperty("season") && parseFileName.season) ||
                (anitomyParse.hasOwnProperty("episode_number") && anitomyParse.episode_number && anitomyParse.hasOwnProperty("anime_title") && anitomyParse.anime_title)
            ) {
    ...
    async createSymlink(filePath, linkPath, metaData) {
        ...
        try {
            ...
            const searchLink = this.store.findLink(linkPath);
            if (searchLink) {
                if (
                    ...
                    if (
                        ...
                        if (searchLink.metaData.hasOwnProperty("quality") && metaData.hasOwnProperty("quality")) {

                            const existingQualityInteger = (typeof searchLink.metaData.quality === "string") ? searchLink.metaData.quality.replace(/\D/g, '') : searchLink.metaData.quality;
                            const newQualityInteger = (typeof metaData.quality === "string") ? metaData.quality.replace(/\D/g, '') : metaData.quality;

                            if (existingQualityInteger < newQualityInteger) { // more is better?
            ...
            if (updateSymlink) {
                try {
                    ...
                    this.store.createLink(linkPath, filePath, metaData);
                } catch (error) {
