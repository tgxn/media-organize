...
class OrganizerLayer {
    ...
    async onFileDeleted(file) {
        logger.log(`File Deleted: "${file}".`);

        const linkExists = this.memory.findLinkWithSource(file);
        if (linkExists) {
            logger.log("Remove Link:", linkExists.linkPath);

            try {
                await fs.unlink(linkExists.linkPath);
                this.memory.deleteLink(linkExists.linkPath);
            } catch (error) {
                logger.log("failed to unlink", linkExists.linkPath, error.toString());
            }
        }
    }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    logger.log("failed to link", filePath, linkPath, error.toString());
                }
            }
        } catch (error) {
            logger.log("failed to create symlink", error.toString());
        }
    }
}
