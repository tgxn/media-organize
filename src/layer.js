...
class OrganizerLayer {
        ...
        const results = await Promise.all(
            files.map(async (filePath) => {
                ...
                if (fileIsMedia) {
                    ...
                    if (fileMediaInfo == null) {
                        logger.info(`failed to determine media info: ${filePath}`, movieOrSeries);
                        return false;
                    }
    ...
    formatSeriesPath(targetMetadata, fileInfo) {
        ...
        if (
            this.configData.seriesCaseFormat &&
    ...
    async createSymlink(filePath, linkPath) {
        try {
            ...
            try {
                ...
                try {
                    if (this.configData.useHardLinks) {
                        await fs.link(filePath, linkPath);
                    } else {
                        await fs.symlink(filePath, linkPath);
                    }
                } catch (error) {
                    logger.error(`failed to link: ${error.toString()}`, filePath, linkPath);
                }
            }
        } catch (error) {
